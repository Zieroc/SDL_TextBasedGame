...
{
	std::shared_ptr<Game> pRet(new Game());
	return pRet;
}
...
bool Game::Initialize(const char* title, int x, int y, int width, int height, int flags)
{
	if(SDL_Init(SDL_INIT_EVERYTHING) == 0)
	{
		...
		m_p_ContentManager = ContentManager::Create(m_p_Renderer);
		m_p_ScreenManager = ScreenManager::Create(m_p_Renderer, m_p_ContentManager);
		m_p_ScreenManager->AddScreen(GameplayScreen::Create(), false);
		return true;
	}
	else
	{
